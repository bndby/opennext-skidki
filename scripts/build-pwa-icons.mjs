...
async function writePng(svg, destination, size) {
	mkdirSync(dirname(destination), { recursive: true });
	await sharp(Buffer.from(svg)).resize(size, size).png({ compressionLevel: 9 }).toFile(destination);
}

async function buildIcons() {
	const anySvg = createIconSvg({ size: 512, padding: 64, radius: 96 });
	const maskableSvg = createIconSvg({ size: 512, padding: 112, radius: 0 });
	const appleSvg = createIconSvg({ size: 180, padding: 22, radius: 40 });

	writeFileSync(join(PUBLIC_DIR, "icon.svg"), anySvg);

	await writePng(anySvg, join(PUBLIC_DIR, "icons", "icon-192.png"), 192);
	await writePng(anySvg, join(PUBLIC_DIR, "icons", "icon-512.png"), 512);
	await writePng(maskableSvg, join(PUBLIC_DIR, "icons", "icon-512-maskable.png"), 512);
	await writePng(appleSvg, join(PUBLIC_DIR, "apple-touch-icon.png"), 180);

	console.log("[build-icons] PNG-иконки PWA записаны.");
}
